...
"""Generate influenza-data.json for the live Chronicling America demo."""
...
MAX_RETRIES = 5
OUTPUT_PATH = Path(__file__).resolve().parent.parent / "influenza-data.json"
USER_AGENT = "Mozilla/5.0 (compatible; vibedemo-fetcher/2.0; +https://github.com/KevinHegg/vibedemo)"
...
def count_for_state(year: int, state_name: str, *, last_request_at: float | None) -> tuple[int, float]:
    params = {
        "fo": "json",
        "c": "1",
        "at": "pagination",
...
def build_data() -> dict:
    ...
    for year in YEAR_RANGE:
        ...
        print(f"National {year}: {count}", file=sys.stderr, flush=True)
    ...
    for abbreviation, state_name in STATE_NAMES.items():
        ...
        print(f"{MAP_YEAR} {abbreviation} ({state_name}): {count}", file=sys.stderr, flush=True)
    ...
    return {
        ...
        "map_year": MAP_YEAR,
        "source": {
            "name": "Library of Congress Chronicling America collection endpoint",
            "url": BASE_URL,
        },
        "query_params": {
            ...
            "verified_examples": {
                "date": "2026-04-01",
                "baseline_1918": 52852,
                "front_pages_only_1918": 52852,
                "location_state_new_york_1918": 52852,
                "fa_location_new_york_1918": 1421,
            },
            "notes": [
                "Counts come from pagination.of on the loc.gov collection endpoint, with pagination.total as a fallback if needed.",
                "On April 1, 2026, front_pages_only=true did not change the 1918 influenza total relative to the baseline query.",
                "On April 1, 2026, location_state=new york also did not change the 1918 influenza total relative to the baseline query.",
